...
// extrusion_profile_30x30(l=100);

module extrusion_profile_30x30(l=10) {
    size = 30;
    radius = 1.5;
    step = 1;
	linear_extrude(l) {
		union() {
			sub_extrusion_profile_30x30_part(size, radius, step);
			rotate([0,0,90])  sub_extrusion_profile_30x30_part(size, radius, step);
			rotate([0,0,180]) sub_extrusion_profile_30x30_part(size, radius, step);
			rotate([0,0,270]) sub_extrusion_profile_30x30_part(size, radius, step);
		}
	}
}

module sub_extrusion_profile_30x30_part(size=30, radius = 1.5, step=0.5) {
